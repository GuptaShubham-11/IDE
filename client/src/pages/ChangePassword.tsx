  ...
  const handleVerifyOtp = () => {
    ...
    if (otp !== actualOtp || new Date() > new Date(expiresIn)) {
      showAlert(
        ...
        'Check your OTP or its validity.'
      );
      return;
    }
    setStep('password');
      ...
      <div className="w-full max-w-md bg-white rounded-2xl p-8 shadow-2xl border border-border relative overflow-hidden">
        ...
        <div className="space-y-2 text-center">
          ...
          <p className="text-xs text-red-500 mt-1 font-medium">
            ⚠️ Do not refresh this page until the process is complete.
          </p>
        </div>
        ...
        {step === 'otp' && (
          <>
            ...
            <div className="text-sm text-secondaryL text-center mt-3">
              Expires in:{' '}
              <span
                className={`font-medium ${timeLeft > 0 ? 'text-green-600' : 'text-red-600'}`}
              >
                {timeLeft}s
              </span>
              {timeLeft === 0 && (
