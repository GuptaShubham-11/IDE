export const FormatDockerError = (errorLog: string): string => {
    ...
    const errorMatch = errorLog.match(/(SyntaxError|ReferenceError|TypeError|NameError|IndentationError|Exception): (.+)/);
    if (errorMatch) {
        const [, errorType, message] = errorMatch;

        // Try to extract file and line number info
        const lineMatch = errorLog.match(/program\.(js|py|c|cpp|java|go|php):(\d+)/);
