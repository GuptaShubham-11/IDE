...
const signIn = asyncHandler(async (req, res) => {
    ...
    if (!user.verified) {
        ...
        try {
            ...
            await sendOtpOnEmail(title, user.email, VerifyEmail({ name: user.name, otp: user.otp }));
        } catch (error: any) {
...
const reSendVerificationEmail = asyncHandler(async (req, res) => {
    ...
    if (!canSend) {
        throw new ApiError(429, `Too many requests. Please try again after ${retryAfter} minutes.`);
    }
